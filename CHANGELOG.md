...
### Fixed
- **AI Tools – duplicate tool name collision**: Connecting more than one `Buxfer AI Tools` node to the same AI Agent failed with `You have multiple tools with the same name: 'buxfer_listAccounts'`. Every node exposed the discovery helpers under fixed names, so any two nodes collided — even with different resources — and two nodes of the same resource also collided on the main tool.
- **AI Tools – node-name-derived tool names**: Tool names are now derived from the node name (n8n's own `nodeNameToToolName` idiom): `{resource}_{NodeName}` (main), `listAccounts_{NodeName}` and `listTags_{NodeName}` (discovery helpers). The resource is the functional identifier and the node name carries uniqueness + the Buxfer branding — there is deliberately no separate `buxfer_` prefix, which would duplicate the "Buxfer" already in most node names. Renaming a node renames its tools, exactly what the n8n error message suggests.
- **AI Tools – uniqueness under sanitization/truncation**: A short stable hash of the full original node name is appended whenever sanitization alters the name (so punctuation/spacing variants like `Buxfer Expenses` vs `Buxfer_Expenses` can't collide) or the name must be truncated (so two names sharing a long prefix stay distinct). Canonical names (letters/digits/`_`/`-` only) stay clean with no hash. All names stay within the MCP regex and OpenAI's 64-char limit.
- **AI Tools – envelope + hints use the registered name**: The response envelope's `tool` field and all LLM-facing descriptions/error hints now reference the actual registered tool name (previously a stale `buxfer_{resource}` identifier that no longer matched any registered tool).
